...
`contracts/fixtures/remote-relay-v1.json`. Source flags remain disabled by default;
the separately reviewed first production server deployment is active on the exact
`4070pc`-to-`lugos-host` binding, while workstation pulls still require explicit
arming.
The Batch 8 transport is implemented across Mission Control and Fade. The reviewed
first target uses a passkey decision at `knot.newman.foo`, a least-privilege relay on
`lugos-host`, and outbound TLS 1.3/mTLS pull from `4070pc`.
...
leave the workstation. The live activation, one-effect canary, terminal
acknowledgement, redacted event readback, and capsule-body purge are recorded in
`docs/acceptance/batch8-remote-relay-20260829.md`.
